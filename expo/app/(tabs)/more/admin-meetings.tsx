...
export default function AdminMeetingsScreen() {
  ...
  const scheduleMutation = useMutation({
    mutationFn: async ({ meetingId, date }: { meetingId: number; date: string }) => {
      const res = await api.put(`/api/admin/meetings/${meetingId}/schedule?scheduled_date=${encodeURIComponent(date)}`, {});
      return res.data;
    },
    onSuccess: () => {
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen options={{ headerShown: true, title: 'Meeting Management', headerStyle: { backgroundColor: Colors.bgPrimary }, headerTintColor: Colors.textPrimary, headerShadowVisible: false }} />
      <ScrollView
